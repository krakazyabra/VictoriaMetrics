...
* BUGFIX: [vmalert](https://docs.victoriametrics.com/vmalert.html): fix alert firing state in replay mode. Before if firing stage is bigger than single query request range, alerting rule won't be able to fire. See [this issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/5186) for details.
